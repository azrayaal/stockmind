      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500">
            Catat dan pantau stok masuk ke inventori
          </p>
        </div>
        <Button variant="success" className="gap-2" onClick={() => setModalOpen(true)}>
      ...
      <div className="rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
        {incomingTransactions.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <ArrowDownToLine className="h-12 w-12 text-zinc-300" />
            <p className="mt-4 text-sm font-medium">Belum ada transaksi masuk</p>
            <p className="text-xs text-zinc-500">
              Klik tombol di atas untuk mencatat barang masuk
            </p>
          </div>
        ) : (
          <Table>
              ...
              {incomingTransactions.map((tx) => (
                <TableRow key={tx.id}>
                  ...
                  <TableCell className="text-right">
                    <Badge variant="success">+{tx.quantity}</Badge>
                  </TableCell>
